...
export interface OptimizerSteps {
  removeErrors: boolean;
  invalidAction: 'remove' | 'comment';
  duplicateAction: 'remove' | 'comment';
  normalizeFormat: boolean;
  fixOwnerDomain: boolean;
  fixRelationship: boolean;
  fixManagerDomain: boolean;
  managerAction: 'remove' | 'comment';
  verifySellers: boolean;
  sellersAction: 'remove' | 'comment';
  verifyCertAuthority: boolean;
}
...
export class OptimizerService {
  async process(
    ...
    if (steps.fixManagerDomain) {
      ...
      optimizedContent = newLines.join('\n');
      removedCount += removedManagerDomains;
    }
