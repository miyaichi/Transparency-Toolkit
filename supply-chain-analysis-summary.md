...
**ステータス:** ❌ 却下・終了（2026-04-04）

> **却下理由:**
> 本ドキュメントは「INTERMEDIARY チェーンの正しい追跡ロジックの実装」を次のステップとして推奨したが、2026-04-04 の DB データ分析により前提が誤りであったことが判明した。
>
> **実際の制約はロジックではなくデータ**:
> - `supply_chain_hops` の `resolved_depth=3` は 1件のみ（未解決）
> - `resolved_depth=2` の未解決 3,953件のうち 3,584件は sellers.json 取得済みドメインでも seller_id が登録されていないため、ロジックを改善しても解決しない
> - sellers.json を追加取得可能な未取得ドメインは 105件だが、hop=4+ はほぼ実現しない
>
> **結論:** hop=4+ のデータは sellers.json の業界的な登録不足により構造的に存在しない。「中期・長期」として提案した Hop Calculator の実装（Option A/B）および supply_chain_hops テーブルの活用はすべて不要。関連コードおよびテーブルは 2026-04-04 に削除済み。
>
> **正しかった判断:** 「短期推奨」として記載した「現在の推測ベースロジック（seller_type）をそのまま使用」が最終的な正解であった。`getHopCount()` の `1 / 2 / 3+` 表示が実態に最も即した実装。
