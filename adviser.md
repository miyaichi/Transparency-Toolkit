...
ドメインを入力すると、複雑な広告配信メトリクスを解析し、パブリッシャーが改善すべき点を提案するダッシュボード機能です。
2. 主な提供価値
...
# Input Data context
以下の定義に基づいてデータを解釈します：
- avgAdsToContentRatio (A2CR): 20%を超えると広告過多と見なされ、バイヤーに避けられるリスクがあります。
- avg_page_weight / avg_cpu: 数値が高いほどユーザー体験が悪化し、ビューアビリティが低下します。
- id_absorption_rate: 数値が高いほど、ターゲティング精度が高まり収益性が向上します。
- reseller_count / total_supply_paths: 多すぎると透明性が欠如し、パブリッシャーの取り分が減ります（SPOの対象）。
